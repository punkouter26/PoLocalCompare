...
public class GreenStatsCalculatorTests
{
    ...
    [Fact]
    public void ComputeGreenScore_TokensPerWattHour()
    {
        Assert.Equal(500, GreenStatsCalculator.ComputeGreenScore(tokenCount: 1000, energyWh: 2));
    }

    [Fact]
    public void ComputeGreenScore_ZeroEnergy_ReturnsZeroRatherThanDividingByZero()
    {
        // The guard matters: an infinite score would rank a model with no measured energy
        // at the top of the Green Score leaderboard forever.
        Assert.Equal(0, GreenStatsCalculator.ComputeGreenScore(1000, 0));
    }

    [Fact]
    public void ComputeGreenScore_NegativeEnergy_ReturnsZero()
    {
        Assert.Equal(0, GreenStatsCalculator.ComputeGreenScore(1000, -5));
    }

    [Fact]
    public void ComputeGreenScore_ZeroTokens_IsZero()
    {
        Assert.Equal(0, GreenStatsCalculator.ComputeGreenScore(0, 2));
    }

    [Fact]
    public void ComputeGreenScore_MoreTokensForSameEnergy_ScoresHigher()
    {
        var fewer = GreenStatsCalculator.ComputeGreenScore(500, 1.5);
        var more = GreenStatsCalculator.ComputeGreenScore(900, 1.5);

        Assert.True(more > fewer);
    }

    [Fact]
    public void ComputeGreenScore_SameTokensForMoreEnergy_ScoresLower()
    {
        var efficient = GreenStatsCalculator.ComputeGreenScore(800, 1.0);
        var wasteful = GreenStatsCalculator.ComputeGreenScore(800, 4.0);

        Assert.True(wasteful < efficient);
    }

    [Fact]
    public void ComputeGreenScore_RoundsToTwoDecimals()
    {
        var result = GreenStatsCalculator.ComputeGreenScore(1000, 3);
